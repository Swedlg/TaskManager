...
namespace TaskManagerDatabaseImplement.Implements
{
    public class DevelopersStorage : IDevelopersStorage
    {
        ...
        public bool LinkProgramLanguage(DeveloperBindingModel developerModel, ProgramLanguageBindingModel programLanguageModel)
        {
            using (var context = new TaskManagerDatabase())
            {
                DeveloperProgramLanguage element = context.DeveloperProgramLanguage.FirstOrDefault(rec => rec.DeveloperId == developerModel.Id && rec.ProgramLanguageId == programLanguageModel.Id);
