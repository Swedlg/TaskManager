...
namespace TaskManagerPostgresDatabaseImplement.Implements
{
    public class StagesStorage : IStagesStorage
    {
        ...
        public bool LinkDeveloper(StageBindingModel stageModel, DeveloperBindingModel developerModel)
        {
            using (var context = new TaskManagerDatabase())
            {
                StageDeveloper element = null;

                if (context.StageDeveloper.Count() != 0)
                {
                    element = context.StageDeveloper.FirstOrDefault(rec => rec.StageId == stageModel.Id && rec.DeveloperId == developerModel.Id);
                }
                if (element != null)
                {
                    return false;
